...
if st.button('Exibir tabela (Estado)'):
    ...
    try:
      st.dataframe(geo_state_dataframe[['name_muni', 'abbrev_state', 'populacao', 'pib', 'IDH', 'pib_per_capita','app', 'soma', 'max']+app])
    except:
      st.write(geo_state_dataframe[['name_muni', 'abbrev_state', 'populacao', 'pib', 'IDH', 'pib_per_capita','app', 'soma', 'max']+app])
...
if st.button('Estimativa socioeconômica'):
    ...
    for i in lista_app:
      ...
      df_potencial['PIB_per_capita_potencial'].append(round((np.array(df_potencial['PIB_potencial']) / np.array(df_potencial['demanda_potencial']))[0],2))
...
if botao_tendencia:
    ...
    plt.title(f'Proporção mensal {" X ".join(app)} em {retorno_df["geoName"][0]} {data_inicial.split("-")[2]}', fontsize=15)
    ...
    plt.rcParams.update({"font.size": 10})
    plt.title(f'Evolução semanal {" X ".join(app)} no Brasil {data_inicial.split("-")[2]}', fontsize=15)
    plt.box(False)
    plt.xlabel('data')
    plt.ylabel('Google Trends Index')
    plt.ylim(0,100)
